...
data class UserStudyState(
    val title: String = "",
    val weeks: List<WeeklyCurriculum> = emptyList(),
) : UiState
...
class UserStudyViewModel @Inject constructor(
    ...
    */
        ...
        viewModelScope.launch {
            appDataStoreRepository.getUserInfo()
                ...
                .collect { userInfo ->
                    ...
                    val part = record.part.ifBlank { return@collect }
                    load(gisuId, part)
                }
        }
    }

    private fun load(gisuId: Long, part: String) {
        startLoading()
        viewModelScope.launch {
            when (val result = getCurriculumOverviewUseCase(gisuId, part)) {
                is ApiState.Success -> {
                    updateState {
                        copy(
                            title = result.data.curriculumTitle,
    ...
    fun onSubmitClick(itemId: Long, link: String) {
        ...
        updateItem(itemId) { item ->
            ...
            when (item.submitState) {
                ...
                SubmitState.CONFIRMING -> item.copy(link = trimmed)
            }
        }
    }
    ...
    private suspend fun submitWorkbook(originalWorkbookId: Long, link: String) {
        if (USE_FAKE_SUBMIT) {
            ...
            updateItem(originalWorkbookId) {
                it.copy(status = StudyStatus.PASS, submitState = SubmitState.IDLE, isExpanded = false)
            }
    ...
    private fun updateItem(itemId: Long, transform: (ActStudyItemUiModel) -> ActStudyItemUiModel) {
        updateState {
    ...
    private fun applyLockRule(list: List<ActStudyItemUiModel>): List<ActStudyItemUiModel> {
        return list.sortedBy { it.week }
            .map { item ->
                item.copy(isExpanded = if (item.isLocked) false else item.isExpanded)
            }
    }
