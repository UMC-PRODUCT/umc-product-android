...
constructor(
    ...
    fun onClickBackPressed(){
        emitEvent(MyContentEvent.ClickBackPressed)
    }
    ...
    fun initShowType(showType: String){
        //오류 핸들
        if (showType.isBlank()) {
            emitEvent(MyContentEvent.ShowErrorToast("오류가 발생했습니다."))
            return
        }
...
sealed interface MyContentEvent : UiEvent {
    object ClickBackPressed : MyContentEvent

    data class ShowErrorToast(val errorMessage : String) : MyContentEvent
}
